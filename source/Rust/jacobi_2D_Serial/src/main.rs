...
impl<T: Num + Default + Clone> Grid<T> {
  pub fn new( _size : usize ) -> Grid<T>{
    Grid{
      size: _size as usize,
      grid: vec![ Default::default(); _size*_size as usize ]
    }
  }
    ...
    if t & 1 == 0 {
      for x in 1..grid_b.size-2 {
        for y in 1..grid_b.size-2 {
          let next =                    ( grid_a.read(x,y-1) +
                     grid_a.read(x-1,y) + grid_a.read(x,y  ) + grid_a.read(x+1,y) +
                                          grid_a.read(x,y+1) ) / 5;
          grid_b.write(x,y, next);
        }
      }
    } else {
      for x in 1..grid_a.size-2 {
        for y in 1..grid_a.size-2 {
          let next =                    ( grid_b.read(x,y-1) +
                     grid_b.read(x-1,y) + grid_b.read(x,y  ) + grid_b.read(x+1,y) +
                                          grid_b.read(x,y+1) ) / 5;
          grid_a.write(x,y, next);
        }
      }
    }
  }
